...
async function startServer() {
  ...
  // 301 Redirect non-www to www and http to https for xedep.pro
  app.use((req, res, next) => {
    const host = req.headers.host || "";
    const proto = req.headers["x-forwarded-proto"] || req.protocol;
    
    if (host === "xedep.pro") {
      return res.redirect(301, `https://www.xedep.pro${req.originalUrl}`);
    }
    
    if (host === "www.xedep.pro" && proto === "http") {
      return res.redirect(301, `https://www.xedep.pro${req.originalUrl}`);
    }
